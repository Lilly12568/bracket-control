...
## Publish it at a `github.io` address

This version publishes the visible application to GitHub Pages and deploys only
the two data-fetching endpoints as a hidden Cloudflare Worker. The Worker URL is
compiled into the application and never replaces the `github.io` address in the
browser.

### One-time setup

1. Create a GitHub repository and put this project's files in it. Use `main` as
   the default branch.
2. In **GitHub repository → Settings → Pages**, set **Source** to **GitHub
   Actions**.
3. Create a free Cloudflare account. Open **Workers & Pages** once and create a
   `workers.dev` subdomain if Cloudflare asks you to choose one. This address is
   used only behind the scenes.
4. In Cloudflare, create an API token from the **Edit Cloudflare Workers**
   template and copy your account ID.
...
6. Push to `main`, or open **Actions → Deploy Bracket Control → Run workflow**.
   The workflow deploys the API, captures its address automatically, builds the
   static application with the correct repository path, and publishes it.

For a repository named `Bracket-Control`, the public address will be:

```text
https://YOUR-GITHUB-USERNAME.github.io/Bracket-Control/
```

If the repository itself is named `YOUR-GITHUB-USERNAME.github.io`, the public
address will instead be the shorter:

```text
https://YOUR-GITHUB-USERNAME.github.io/
```

Every later push to `main` updates both parts automatically. Tournament edits
remain local to each browser, just as they do in the desktop version.

## Start it

On Windows, double-click `start-bracket-control.cmd`. Or run:
